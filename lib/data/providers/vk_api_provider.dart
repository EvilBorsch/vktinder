// --- File: lib/data/providers/vk_api_provider.dart ---
...
class VkApiProvider extends getx.GetxService {
  ...
  dynamic _handleResponse(Response response) {
    if (response.statusCode == 200 && response.data != null) {
      if (response.data['error'] != null) {
        ...
        if (errorCode == 5) {
          // Authorization failed (bad token)
          throw DioException(
      ...
      if (response.data is Map && response.data.containsKey('response')) {
        ...
        print(
            'VK API Success Response (no "response" key or not a map): ${response.data}');
        return response
            .data; // Return data even if 'response' key is missing (e.g., for boolean results)
      }
    } else {
      throw DioException(
        requestOptions: response.requestOptions,
  ...
  Future<int?> _getMockGroupIdByScreenName(String screenName) async {
    ...
    switch (screenName.toLowerCase()) {
      ...
      case "recipes_vk":
        return 10;
      case "team":
        return 25504844; // Example: VK Team group
      case "durov":
        return 1; // Pavel Durov (as group/page)
      case "designhunters":
  ...
  Future<List<Map<String, dynamic>>> _getMockCitiesByName(
    ...
    final predefinedCities = {
      ...
      // Example where input might be ambiguous
    ...
    for (var name in cityNames) {
      ...
      if (predefinedCities.containsKey(lowerName)) {
        results.add({
          "id": predefinedCities[lowerName]!['id'],
          "title": predefinedCities[lowerName]!['title']
        });
  ...
  Future<List<VKGroupUser>> _getMockSearchUsers({
    ...
    if (offset >= 100) {
      // Simulate reaching the end of mock results
    ...
    final random = Random(groupId ??
        0 + (cityId ?? 0) + offset); // Seeded random for consistency per page
    for (int i = 0; i < count; i++) {
      final uniqueIdBase =
          (groupId ?? 0) * 10000 + (cityId ?? 0) * 100 + offset + i;
      final uniqueId = random.nextInt(900000) + uniqueIdBase; // Add randomness

      // Simulate filter matching (loosely)
      bool matchesCity = cityId == null ||
          (uniqueId % 5 == cityId % 5); // Simulate city match rate
      int mockAge = 18 + random.nextInt(30); // Age 18-47
      bool matchesAge = (ageFrom == null || mockAge >= ageFrom) &&
          (ageTo == null || mockAge <= ageTo);
      ...
      if (matchesCity && matchesAge && matchesSex) {
        mockUsers.add(VKGroupUser.fromJson({
          // Using simplified structure for mock
          "id": uniqueId,
  ...
  Future<VKGroupUser> getFullProfile(String vkToken, String userID) async {
    ...
    if (vkToken.isEmpty || userID.isEmpty) {
      throw ArgumentError(
          "VK Token and User ID must be provided for getFullProfile.");
    }
    try {
      ...
      if (responseData == null ||
        ...
        print(
            "Warning: users.get response data is null, not a list, or empty.");
      ...
      if (e.response?.data?['error']?['error_code'] == 15 ||
          e.response?.data?['error']?['error_code'] == 30) {
        throw Exception(
            'Не удалось загрузить профиль: Доступ к профилю ограничен.');
      }
      throw Exception(
          'Не удалось загрузить профиль: ${e.message?.split(':').last.trim() ?? 'Ошибка сети'}');
    } catch (e, stackTrace) {
  ...
  Future<List<String>> getUserPhotos(String vkToken, String userID) async {
    ...
    if (vkToken.isEmpty || userID.isEmpty) {
      print(
          "Warning: VK Token or User ID missing for getUserPhotos. Returning empty list.");
    ...
    try {
      ...
      if (responseData == null || responseData['items'] == null) {
        print(
            "Warning: photos.get response data or items list is null for user $userID.");
        return [];
      }
      ...
      if (e.response?.data?['error']?['error_code'] == 15 ||
          e.response?.data?['error']?['error_code'] == 200 ||
          e.response?.data?['error']?['error_code'] == 30) {
        print(
            "Could not fetch photos for user $userID due to privacy settings or access error.");
      } else {
  ...
  Future<bool> sendMessage(
    ...
    // --- MOCK SWITCH ---
    if (_useMockData)
      return _sendMockMessage(userId, message); // <--- Corrected Call
    // --- END MOCK SWITCH ---
    // ... (rest of the method remains the same)
    ...
    try {
      ...
      if (response.data != null && response.data['error'] != null) {
        ...
        final errorCode = error['error_code'] ?? -1;
        final errorMessage =
            error['error_msg'] ?? 'Unknown VK send message error';
      ...
      if (responseData != null && responseData is int) {
        ...
        print(
            "Message send call succeeded, but response was not the expected message ID: $responseData");
        getx.Get.snackbar(
          // Use alias here
          'Ошибка',
  ...
  Future<List<int>> getUserSubscriptionIds(
      String vkToken, String userId) async {
    // --- MOCK SWITCH ---
    if (_useMockData)
      return _getMockUserSubscriptionIds(userId); // <--- Corrected Call
    // --- END MOCK SWITCH ---
    // ... (rest of the method remains the same)
    if (vkToken.isEmpty || userId.isEmpty) {
    ...
    try {
      ...
      if (e.response?.data?['error']?['error_code'] == 15 ||
          e.response?.data?['error']?['error_code'] == 30) {
        print(
            "Could not fetch subscriptions for user $userId due to privacy settings.");
      } else {
  ...
  Future<List<VKGroupInfo>> getGroupsById(
      String vkToken, List<String> groupIds) async {
    // --- MOCK SWITCH ---
    if (_useMockData)
  ...
  Future<VKGroupInfo?> getGroupInfoByScreenName(
      String vkToken, String screenNameOrUrl) async {
    // --- MOCK SWITCH ---
    if (_useMockData) {
      final mockId = await _getMockGroupIdByScreenName(
          _extractScreenName(screenNameOrUrl));
      if (mockId != null) {
    ...
    if (vkToken.isEmpty || screenNameOrUrl.isEmpty) {
      print(
          "Error: VK Token or screenName/URL is missing for getGroupInfoByScreenName.");
      return null;
    }
    ...
    if (screenName.isEmpty) {
      print(
          "Error: Could not extract valid screen name from '$screenNameOrUrl'.");
      return null;
    }
    ...
    try {
      ...
      if (responseData is List && responseData.isNotEmpty) {
        ...
        return VKGroupInfo.fromJson(groupData, sourceUrl: screenNameOrUrl);
          ...
          responseData.containsKey('groups') &&
          responseData['groups'] is List &&
          responseData['groups'].isNotEmpty) {
        final groupData = responseData['groups'][0] as Map<String, dynamic>;
        return VKGroupInfo.fromJson(groupData, sourceUrl: screenNameOrUrl);
      } else {
        print(
            "Group with screen_name '$screenName' not found or API response format unexpected.");
        return null;
      }
    } on DioException catch (e) {
      ...
      print(
          "Error parsing group info for screen name '$screenName': $e\n$stackTrace");
      return null;
    }
  }
  ...
  String _extractScreenName(String input) {
    ...
    if (uri != null &&
        (uri.host.contains('vk.com') || uri.host.contains('vkontakte.ru'))) {
      if (uri.pathSegments.isNotEmpty) {
        return uri.pathSegments.last;
      }
    }
    // Handle cases like 'public12345', 'club12345' - VK API often handles these directly
    // Allow screen names with digits initially like 'club123' or 'public123'
    ...
    print(
        "Warning: Input '$input' doesn't look like a valid VK URL or screen name.");
    // Return the input anyway, VK API might still resolve it in some cases.
    return input;
    // return ''; // Previous stricter version
  }

  Future<Map<String, int>> getCityIdsByNames(
    ...
    if (_useMockData) {
      ...
      final mockMap = {
        for (var city in mockCities)
          city['title'].toString().toLowerCase(): city['id'] as int
      }; // Map found titles to IDs
    ...
    final uniqueLowerTrimmedNames = cityNames
        .map((n) => n.toLowerCase().trim())
        .where((n) => n.isNotEmpty)
        .toSet(); // Process unique non-empty names
      ...
      try {
        ...
        if (responseData != null &&
          ...
          if (cityId != null &&
            ...
            print(
                "Warning: City '$lowerTrimmedCityName' found but has null ID/title or wrong country ($foundCountryId).");
          }
        } else {
        ...
        print(
            "DioError getting city ID for '$lowerTrimmedCityName': ${e.message}");
      } catch (e, stackTrace) {
        print(
    ...
    print(
        "Resolved ${finalResultMap.length} cities out of ${cityNames.length} requested names.");
    return finalResultMap;
  }

  // Helper to convert user data from response, keeping groupURL
  VKGroupUser convertUserFromResponse(
      Map<String, dynamic> userData, String? groupURL) {
    userData["groupURL"] = groupURL; // Attach the source group URL
  ...
  Future<List<VKGroupUser>> searchUsers({
    ...
    int? ageFrom,
    int? ageTo,
    int sex = 1, // 0-any, 1-female, 2-male
    int count = 100,
    int offset = 0,
    String? groupURL, // Pass the groupURL for context
  }) async {
    // --- MOCK SWITCH ---
    if (_useMockData) {
      return _getMockSearchUsers(
        groupId: groupId,
        cityId: cityId,
        ageFrom: ageFrom,
        ageTo: ageTo,
    ...
    // The 'status' parameter in users.search maps to relationship status (relation field)
    // Since we need to filter by 0 OR 6, and the API only takes one status,
    // we DON'T add the status param here. We will filter client-side if needed.
    // if (relationStatus != null) queryParams['status'] = relationStatus;
    ...
    try {
      ...
      final List usersList = responseData['items'] ?? [];
      print(
          "users.search returned ${usersList.length} users (offset: $offset).");
      ...
      if (e.response?.data?['error']?['error_code'] == 15 ||
          e.response?.data?['error']?['error_code'] == 30) {
        throw Exception(
            'Ошибка поиска: Доступ к некоторым данным ограничен приватностью.');
      }
      throw Exception(
          'Ошибка поиска пользователей: ${e.message?.split(':').last.trim() ?? 'Ошибка сети'}');
    } catch (e, stackTrace) {
